...
beforeAll(async () => {
  const config = loadConfig({
    ...
    ANTHROPIC_API_KEY: 'sk-ant-stubbed-in-stop-e2e',
  });
...
describe('a turn the client stops partway', () => {
  ...
  it('attributes the stopped answer and records the usage it was told about', async () => {
    ...
    expect(stored.provenance?.modelId).toBe('claude-sonnet-4-5');
    // Zero, because the provider reports the output count on message_delta and
