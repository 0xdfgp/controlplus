...
The boundary is enforced. dependency-cruiser runs inside npm test with three
error-severity rules, and a pre-write hook blocks a forbidden import before it
lands. Run npm run lint:boundaries and it either passes or names both layers in
the failure. The code was written by a coding agent, and an agent under time
pressure will import an SDK into the domain with a good excuse. The rule went in
during the first slice for that reason.
...
typed parts, so adding video later means a new part type and not a new message
type.

One write per turn, when the turn closes. If the process dies mid-stream, the
partial is lost. That is a deliberate trade. The brief does not name that
failure, and the user reads it as "it cut off, I will ask again". Making it
survivable is about half an hour of work and a lifecycle invariant. The door is
open.

## Providers

Two ports, text generation and transcription, both returning an AsyncIterable. A
voice turn uses two providers by design, so a single provider port would have
...
Weak point, stated because it is one. The transcription port has no product
caller, since voice is transcribed on the device. It exists for the evaluation.
The split is still right, because the two ports have different implementations
and different failure modes. But the composition root no longer proves it.
...
Two things cost time here. Hermes does not implement ReadableStream on fetch and
...
a business rule into an adapter. The bound is on count and not on size, so long
...
count. Family member names are deliberately absent, so a name that is not in the
contract cannot leak into an answer about someone else. It was cut for time and
is in the left-out list.
...
figure built on the first two would have been a tenth of the real spend. Pricing
lives in a domain catalogue that prices the three components separately, because
providers bill them separately. It keys on the dated model snapshot the provider
actually answers on and not the alias that was requested, which would otherwise
have reported every real turn as free. The endpoint that sums a whole
conversation is not built.
...
models and makes the provenance record untrue. What makes that a claim and not a
hope is that two adapters, Anthropic and Gemini, pass the same shared contract
suite today. Adding the second to the composition root is wiring.

Logs keep message content with sensitive data masked, not excluded. Excluding it
was my first instinct and it was wrong. Without seeing what was said and
returned there is no way to investigate a bad answer or catch a prompt
...
interface and not a first-screen banner, because someone of 80 forgets
...
ran against the real provider rather than against fixtures we wrote ourselves.

Google's migration guide documents one usage shape and the API returns another,
and the SDK types declare a parameter the server rejects. In the other
direction, a parameter the docs describe correctly was rejected six times
because it was being sent at the wrong nesting level. Neither the docs nor the
types are evidence for this API.
